...
using System.Linq;
using Signatures.Types;

namespace Signing.Controllers;
...
public class SigningController : ControllerBase
{
    ...
    {
        return _client.AddSignatory(input);
    }

    [HttpPost(Name = "CloseSignatureOrder")]
    public Output.SignatureOrderResult CloseSignatureOrder([FromBody]string signatureOrderID)
    {
        return _client.CloseSignatureOrder(signatureOrderID);
    }

    [HttpPost(Name = "CancelSignatureOrder")]
    public Output.SignatureOrderResult CancelSignatureOrder([FromBody]string signatureOrderID)
    {
        return _client.CancelSignatureOrder(signatureOrderID);
    }

    [HttpPost(Name = "CreateSignatureOrder")]
    public async System.Threading.Tasks.Task<Output.SignatureOrder> CreateSignatureOrder([FromBody]Arguments.CreateOrder input)
    {
        return await _client.CreateSignatureOrder(input);
    }
    [HttpPost(Name = "SignatureOrder")]
    public async System.Threading.Tasks.Task<Output.SignatureOrderInfo> SignatureOrder([FromBody]string orderId)
    {
        return await _client.SignatureOrder(orderId);
    }
    [HttpPost(Name = "AddSignatories")]
    public Output.SignatoryRef [] AddSignatories([FromBody]Arguments.AddSignatories input)
    {
        return _client.AddSignatories(input);
    }
}
