...
_NOTE: This documentation has been taken directly from the documentation generated by `lein new re-frame <app-name>`_
...
* [`src/cljs_auth/`](src/cljs_auth/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs_auth/core.cljs): contains the SPA entry point, `init`
* [`test/cljs_auth/`](test/cljs_auth/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `cljs-re-frame` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_auth/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-re-frame.example
