...
export function Navigation_SidePanel_AB71DB6(navigation_SidePanel_AB71DB6InputProps) {
    ...
    const createMenuItem = (view, count) => {
        let elms;
        return ["children", Interop_reactApi.Children.toArray([(elms = toList(delay(() => {
            let arg10;
    ...
    const menuItems = collect((tupledArg) => {
        let props_4;
        const view_1 = tupledArg[0];
        const notification = tupledArg[1];
        ...
        return ofArray([(props_4 = ofArray([["className", className], ["onClick", (_arg1) => {
            setView(view_1);
        }], createMenuItem(view_1, notification)]), createElement("div", createObj(Helpers_combineClasses("panel-block", props_4)))), createElement("br", {})]);
